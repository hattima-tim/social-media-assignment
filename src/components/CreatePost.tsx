...
import { apiFetch, uploadImage } from "@/lib/client";
...
const GLOBE_SVG = (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20">
    <circle cx="10" cy="10" r="8.2" stroke="#666" strokeWidth="1.4" />
    <path stroke="#666" strokeWidth="1.4" d="M1.8 10h16.4" />
    <path stroke="#666" strokeWidth="1.4" d="M10 1.8c2.4 2.2 3.7 5 3.7 8.2s-1.3 6-3.7 8.2c-2.4-2.2-3.7-5-3.7-8.2S7.6 4 10 1.8z" />
  </svg>
);
const LOCK_SVG = (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20">
    <rect width="13.4" height="9.4" x="3.3" y="8.4" stroke="#666" strokeWidth="1.4" rx="2.6" />
    <path stroke="#666" strokeWidth="1.4" d="M6.6 8.4V6.1a3.4 3.4 0 116.8 0v2.3" />
    <circle cx="10" cy="13.1" r="1.3" fill="#666" />
  </svg>
);
const POST_SVG = (
...
function AudienceSelect({
  ...
  return (
    <div className={`app-audience ${className}`}>
      <span className="app-audience-icon">{value === "PUBLIC" ? GLOBE_SVG : LOCK_SVG}</span>
      <select
        className="app-audience-select"
...
export function CreatePost() {
  ...
  async function submit() {
    if (submitting || (!content.trim() && !file)) return;
    setSubmitting(true);
    setError("");
    try {
  ...
  const postDisabled = submitting || (!content.trim() && !file);

  return (
    <div className="_feed_inner_text_area  _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
      <div className="_feed_inner_text_area_box">
        <div className="_feed_inner_text_area_box_image">
          <Avatar user={me} size={40} className="_txt_img" />
        </div>
        ...
        <AudienceSelect value={visibility} onChange={setVisibility} className="app-audience-inline" />
      </div>
      ...
      <div className="_feed_inner_text_area_bottom">
        <div className="_feed_inner_text_area_item">
          <div className="_feed_inner_text_area_bottom_photo _feed_common">
            <button type="button" className="_feed_inner_text_area_bottom_photo_link" onClick={() => fileInput.current?.click()}>
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">{PHOTO_SVG}</span>
              Photo
            </button>
          </div>
          <div className="_feed_inner_text_area_bottom_video _feed_common">
            <button type="button" className="_feed_inner_text_area_bottom_photo_link">
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">{VIDEO_SVG}</span>
              Video
            </button>
          </div>
          <div className="_feed_inner_text_area_bottom_event _feed_common">
            <button type="button" className="_feed_inner_text_area_bottom_photo_link">
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">{EVENT_SVG}</span>
              Event
            </button>
          </div>
          <div className="_feed_inner_text_area_bottom_article _feed_common">
            <button type="button" className="_feed_inner_text_area_bottom_photo_link">
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">{ARTICLE_SVG}</span>
              Article
            </button>
          </div>
        </div>
        <div className="_feed_inner_text_area_btn">
          <button type="button" className="_feed_inner_text_area_btn_link" onClick={submit} disabled={postDisabled}>
            {POST_SVG} <span>{submitting ? "Posting..." : "Post"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="_feed_inner_text_area_bottom_mobile">
        <div className="_feed_inner_text_mobile">
          <div className="_feed_inner_text_area_item">
            ...
            <AudienceSelect value={visibility} onChange={setVisibility} className="app-audience-bar" />
          </div>
          <div className="_feed_inner_text_area_btn">
            <button type="button" className="_feed_inner_text_area_btn_link" onClick={submit} disabled={postDisabled}>
              {POST_SVG} <span>{submitting ? "Posting..." : "Post"}</span>
